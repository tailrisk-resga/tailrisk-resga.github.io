...
@njit
def panel_block_bootstrap(n_time: int, n_units: int, n_boot: int, block_len: int) -> Iterator[Tuple[np.ndarray, np.ndarray]]:
    for _ in range(n_boot):
        unit_indices = np.random.choice(n_units, size=n_units, replace=True)
        time_indices = []
        while len(time_indices) < n_time:
            t_start = np.random.randint(0, n_time - block_len + 1)
            block = list(range(t_start, t_start + block_len))
            time_indices.extend(block)

        time_indices = np.array(time_indices[:n_time]) 

        yield unit_indices, time_indices
...
class ModelConfidenceSet:
    ...
    def compute(self) -> None:
        if self.panel:
            n_units, n_time, m0 = self.losses.shape

            mloss = self.losses.mean(axis=(0, 1), keepdims=True)  # shape (1, m0)
            dij_bar = mloss - mloss.T                              # shape (m0, m0)

            dij_bar_boot = np.zeros((self.n_boot, m0, m0))
            itr = enumerate(panel_block_bootstrap(n_time, n_units, self.n_boot, self.block_len))
            if self.show_progress:
                itr = tqdm(itr, total=self.n_boot, desc="Bootstrapping")

            for i, (ui, ti) in itr:
                bloss = self.losses[ui][:, ti, :].mean(axis=(0, 1), keepdims=True)
        ...
        for i in itr:
            ...
            pvals[i] = self.pval_fn(
                z0[(slice(None),) + np.ix_(included, included)],
            ...
            di_bar_boot = (
                dij_bar_boot[(slice(None),) + np.ix_(included, included)].mean(axis=1) * scale
            )
